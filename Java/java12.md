...
- **Wrapper class**
  ...
  - Boolean, Byte와 같이 기본형 타입의 이름 첫 글자가 대문자인 것인 **래퍼 클래스**
...
- **예외 처리(Exception Handling)**
  ...
  ```
  실행 도중에 발생할 수 있는 잠재적은 오류까지 검사할 수 없기 때문에 컴파일은 잘 되었어도 실행 중에 에러에 의해서 잘못된 결과를 얻거나 프로그램이 비정상적으로 종료될 수 있음

  프로그램이 갑자기 실행을 멈추고 종료되는 경우 등이 이러한 경우
  ...
  예외는 발생하더라도 프로그래머가 이에 대한 적절한 코드를 미리 작성해 놓음으로써 프로그램의 비정상적인 종요 방지 가능
  ```
